...
Last updated: 2026-09-25 (сессия 5 — ПУБЛИЧНЫЙ HTTPS-ДОСТУП + ФИКСЫ ЛЕКАРСТВ. (1) Backend выведен наружу через Caddy: `https://health.annagrv17.ru` (Let's Encrypt авто-TLS, автопродление). Домен annagrv17.ru на reg.ru (A→178.105.193.136, AAAA→2a01:4f8:c015:67b4::1). Заменил вход по tailscale/VPN на прямой HTTPS. iOS baseURL → https. (2) ВАЖНО про Caddy→backend: бэкенд теперь слушает `AGHEALTH_HTTP_HOST=0.0.0.0` (было 100.123.202.44), а Caddy проксирует на `127.0.0.1:8791` (loopback, НЕ tailscale) — иначе крупные ответы (exercises ~133КБ) шли через tailscale-интерфейс и рвались по таймауту. (3) Лекарства: отметка/отмена приёма ЗА КОНКРЕТНЫЙ ДЕНЬ (backdating) — единый sheet с календарём (mark/undo), быстрое «Принять»=сегодня, накопительный итог корректен. (4) Багфиксы: `?date=` кодировался в `%3F` через appendingPathComponent → 404 при отмене (фикс: buildURL); вычисление «сегодня» переведено с UTC на локальную TZ (совпадает с backend Europe/Moscow); убран `waitsForConnectivity=true` (давал каскад -999 cancelled на параллельных запросах главной). (5) Логирование backend расширено на ВСЕ запросы (было только nutrition); включён Caddy access-log. **ОТКРЫТО:** экран упражнений может не грузиться у Анны — диагностика в процессе (сервер отдаёт 200/133КБ/~20мс, причина уточняется по Caddy+backend логам).)

_Previous: 2026-09-22 (сессия 4 — НОВЫЙ домен ПИТАНИЕ: реальный food tracker (FatSecret/Manual → backend → DB → iOS → дневная/недельная сводка → Dashboard). Заменил mock-заглушку питания и удалил legacy-импорт Excel/CSV. 163/163 backend-теста. FatSecret требует РУЧНОГО действия Анны (регистрация + credentials + IP-whitelist) — до этого работает ручной ввод и вся аналитика.)_
