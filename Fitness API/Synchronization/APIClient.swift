...
final class APIClient {
    ...
    // Собственная сессия. Доступ идёт по прямому HTTPS (health.annagrv17.ru) — без
    // tailscale-релея. Щедрые таймауты + waitsForConnectivity + ретраи оставлены как
    // запас на моргающую мобильную сеть, чтобы тяжёлые разделы не падали с ложным
    // «нет связи» при кратком обрыве.
    ...
    init(baseURL: URL, token: String) {
        ...
        // Диагностика доказала: два параллельных запроса (parent+picker) на /exercises
        // коллапсировали HTTP/2-коннект (сервер 200/11мс, но клиент не получал → timeout -1001).
        // Основной фикс — устранён дублирующий запрос в UI (теперь грузит только picker).
